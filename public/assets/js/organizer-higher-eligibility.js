...
function canMarkCandidate(item) {
    return ["CHUA_DANH_DAU", "TU_CHOI"].includes(candidateStatus(item));
}
...
function syncSourceTournamentOptions(items) {
    items.forEach((item) => {
        const id = Number(item.idgiaidau_nguon || 0);
        if (id > 0) {
            sourceTournamentOptions.set(id, item.tengiaidau_nguon || `Giải #${id}`);
        }
    });
...
async function loadData() {
    ...
    try {
        ...
        syncSourceTournamentOptions(candidates);
        renderCandidates();
        renderIncoming();
    } catch (error) {
        candidates = [];
            ...
            <tr>
                ...
                <td>
                    <div class="row-actions">
                        <button class="btn" type="button" data-action="view" data-achievement-id="${Number(item.idthanhtich)}" data-target-id="${Number(item.idgiaidau_dich)}">Xem</button>
                    </div>
                </td>
            </tr>
        `;

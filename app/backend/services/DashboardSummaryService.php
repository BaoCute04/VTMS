...
final class DashboardSummaryService
{
    ...
    private function athlete(int $accountId): array
    {
        ...
        $summary = $athleteId > 0 ? $this->athleteStats($athleteId) : ['matches' => 0, 'points' => 0, 'serves' => 0, 'blocks' => 0];
        ...
        return $this->baseDashboard(
            ...
            'Theo doi doi bong, doi hinh, lich thi dau ca nhan, thong ke va cac yeu cau ca nhan.',
            'Nam ro lich thi dau va trang thai ca nhan',
            'Xem lich thi dau lien quan, thong tin doi bong dang tham gia, loi moi doi bong va thong ke thi dau cua ban.',
            ['label' => 'Lich thi dau ca nhan', 'href' => '/van-dong-vien/lich-thi-dau-ca-nhan'],
            ['label' => 'Thong ke ca nhan', 'href' => '/van-dong-vien/thong-ke-ca-nhan'],
            [
                $this->stat('Tran lien quan', count($schedule), 'Trong lich ca nhan', 'blue', 'LT'),
                $this->stat('Diem ghi nhan', $summary['points'], 'Tong diem', 'green', 'D'),
            ...
            [
                ...
                'items' => [
                    ...
                    ['label' => 'Tran da co thong ke', 'value' => (string) $summary['matches']],
                ],
            ],
            [
                ...
                ['label' => 'Thong ke ca nhan', 'href' => '/van-dong-vien/thong-ke-ca-nhan', 'desc' => 'Xem chi so thi dau.'],
    ...
    private function spectator(): array
    {
        $schedule = $this->publicScheduleRows();
        $rankingRows = $this->latestPublicRankingRows();
        $nextMatch = $schedule[0] ?? null;
        ...
        return $this->baseDashboard(
            ...
            'Khan gia',
            'Trang chu khan gia',
            'Theo doi doi bong, lich thi dau, ket qua va bang xep hang da cong bo.',
            'Theo doi thong tin giai dau cong khai',
            'Tat ca doi bong, lich thi dau, ket qua va bang xep hang duoc hien thi sau khi ban to chuc cong bo.',
            ['label' => 'Lich thi dau', 'href' => '/khan-gia/lich-thi-dau'],
            ['label' => 'Bang xep hang', 'href' => '/khan-gia/bang-xep-hang'],
            [
                $this->stat('Doi bong cong khai', $this->countPublicTeams(), 'Dang hoat dong', 'blue', 'DB'),
                $this->stat('Tran da cong bo', $this->countPublicMatches(), 'Co the theo doi', 'green', 'TD'),
                $this->stat('Ket qua', $this->countPublicResults(), 'Da cong bo', 'purple', 'KQ'),
                $this->stat('Bang xep hang', $this->countPublicRankings(), 'Da cong bo', 'orange', 'XH'),
            ],
            [
                'title' => 'Lich thi dau cong khai',
                'subtitle' => 'Cac tran sap dien ra va dang duoc cong bo.',
                'badge' => count($schedule) . ' tran',
                'type' => 'table',
                'columns' => ['Thoi gian', 'Giai dau', 'Tran dau', 'Dia diem'],
                'rows' => array_map(fn (array $row): array => [
                    $this->dateTime($row['thoigianbatdau'] ?? null),
                    (string) $row['tengiaidau'],
                    trim((string) $row['doi1'] . ' vs ' . (string) $row['doi2']),
                ...
                'empty' => 'Chua co lich thi dau cong khai.',
            ],
            [
                'title' => 'Bang xep hang nhanh',
                'subtitle' => 'Bang xep hang da cong bo moi nhat.',
                'badge' => 'Top 5',
                'type' => 'ranking',
                'items' => array_map(fn (array $row): array => [
                    'rank' => (int) $row['hang'],
                    ...
                    'meta' => (int) $row['thang'] . ' thang - ' . (int) $row['thua'] . ' thua',
                    'value' => (int) $row['diem'] . 'd',
                    'href' => '/khan-gia/bang-xep-hang',
                ], $rankingRows),
                'empty' => 'Chua co bang xep hang cong khai.',
            ],
            [
                ['label' => 'Doi bong', 'href' => '/khan-gia/doi-bong', 'desc' => 'Xem cac doi bong cong khai.'],
                ['label' => 'Lich thi dau', 'href' => '/khan-gia/lich-thi-dau', 'desc' => 'Theo doi lich da cong bo.'],
                ['label' => 'Ket qua', 'href' => '/khan-gia/ket-qua', 'desc' => 'Xem ket qua tran dau.'],
                ['label' => 'Bang xep hang', 'href' => '/khan-gia/bang-xep-hang', 'desc' => 'Xem thu hang hien hanh.'],
            ],
            $this->matchHeroCard($nextMatch)
        );
    }
    ...
    private function localizeDashboard(mixed $value): mixed
    {
        ...
        static $map = [
            ...
            'Tat ca doi bong, lich thi dau, ket qua va bang xep hang duoc hien thi sau khi ban to chuc cong bo.' => 'Tất cả đội bóng, lịch thi đấu, kết quả và bảng xếp hạng được hiển thị sau khi ban tổ chức công bố.',
            ...
            'Theo doi doi bong, lich thi dau, ket qua va bang xep hang da cong bo.' => 'Theo dõi đội bóng, lịch thi đấu, kết quả và bảng xếp hạng đã công bố.',
            ...
            'Theo doi thong tin giai dau cong khai' => 'Theo dõi thông tin giải đấu công khai',
            ...
            'Trang chu khan gia' => 'Trang chủ khán giả',
            ...
            'Doi bong cong khai' => 'Đội bóng công khai',
            'Tran da cong bo' => 'Trận đã công bố',
            'Co the theo doi' => 'Có thể theo dõi',
            'Da cong bo' => 'Đã công bố',
            'Lich thi dau cong khai' => 'Lịch thi đấu công khai',
            'Cac tran sap dien ra va dang duoc cong bo.' => 'Các trận sắp diễn ra và đang được công bố.',
            'Dia diem' => 'Địa điểm',
            'Chua co lich thi dau cong khai.' => 'Chưa có lịch thi đấu công khai.',
            'Bang xep hang da cong bo moi nhat.' => 'Bảng xếp hạng đã công bố mới nhất.',
            'Chua co bang xep hang cong khai.' => 'Chưa có bảng xếp hạng công khai.',
            'Xem cac doi bong cong khai.' => 'Xem các đội bóng công khai.',
            'Theo doi lich da cong bo.' => 'Theo dõi lịch đã công bố.',
            'Xem ket qua tran dau.' => 'Xem kết quả trận đấu.',
            'Xem thu hang hien hanh.' => 'Xem thứ hạng hiện hành.',
    ...
    private function matchHeroCard(?array $match): ?array
    {
        if ($match === null) {
            return [
        ...
        return [
            ...
            'href' => (int) ($match['idtrandau'] ?? 0) > 0 ? $this->matchHref((int) $match['idtrandau']) : '#',
        ];
    }
    ...
    private function matchHref(int $matchId): string
    {
        return '/khan-gia/lich-thi-dau';
    }
    ...
    private function latestPublicRankingRows(): array
    {
        $ranking = $this->one(
            "SELECT bxh.idbangxephang
             FROM Bangxephang bxh
             JOIN Giaidau gd ON gd.idgiaidau = bxh.idgiaidau
             WHERE bxh.trangthai = 'DA_CONG_BO'
    ...
    private function publicScheduleRows(): array
    {
        return $this->rows(
            "SELECT td.*, gd.tengiaidau, d1.tendoibong AS doi1, d2.tendoibong AS doi2, sd.tensandau
             FROM Trandau td
    ...
    private function countPublicTeams(): int
    {
        return $this->count('Doibong', "trangthai = 'HOAT_DONG'");
    }

    private function countPublicMatches(): int
    {
        return $this->scalarInt(
            "SELECT COUNT(*) AS total
             FROM Trandau td
    ...
    private function countPublicResults(): int
    {
        return $this->count('Ketquatrandau', "trangthai = 'DA_CONG_BO'");
    }

    private function countPublicRankings(): int
    {
        return $this->count('Bangxephang', "trangthai = 'DA_CONG_BO'");
    }
